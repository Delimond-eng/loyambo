...
public function createReservation($chambre_id)
{
    ...
    $chambre = $emplacement->chambres()->where('id', $chambre_id)->where('statut', 'libre')->orWhere('statut', 'réservée')->first();
    if(!$chambre){
        return redirect()->route('reservation.created')->with('error', 'La chambre sélectionnée n\'est pas disponible pour la réservation.');
    }
    return view('reservation.create_reservation_client', compact('emplacement', 'chambre', 'chambre_id'));
}
