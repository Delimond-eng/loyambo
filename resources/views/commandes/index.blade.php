        ...
        <div class="content-header">
            <div class="d-flex align-items-center justify-content-between">
                ...
                @if (Auth::user()->role==='serveur')
                    <a href="{{ route("orders.portal") }}" class="waves-effect waves-light btn btn-danger btn-sm text-center btn-rounded">+ Nouvelle commande</a>
                @else
                     @canCloseDay
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($commande->chambre_id)
                                                        {{ $commande->chambre->numero }}
                                                    @else
                                                        {{ $commande->table->numero }}
                                                    @endif
                                                </td>
                                                <td>{{ $commande->total_ttc }} {{ $commande->devise }}</td>
                                                <td>{{ $commande->emplacement->libelle }}</td>
                                                <td>{{ $commande->user->name }}</td>
                                                <td>
                                                    @if ($commande->statut === 'en_attente')
                                                        <span class="badge badge-pill badge-warning-light">En attente</span>
                                                        
                                                    @elseif ($commande->statut === 'payée')
                                                        <span class="badge badge-pill badge-success-light">Payée</span>
                                                    @elseif ($commande->statut==="annulée")
                                                        <span class="badge badge-pill badge-danger-light">Annulée</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    <button type="button" class="btn btn-success btn-xs me-1" title="imprimer" @click="printInvoice(data, data.table ? data.table.emplacement : data.chambre.emplacement)"><i class="mdi mdi-printer"></i></button>
                                                   
                                                      @if ($commande->statut=="payée")
                                                          <a href="{{ route('servir.ok',$commande->id) }}" title="Servir" type="button" class="btn btn-warning btn-xs me-1">
                                                        <i  class="fa fa-glass"></i>
                                                        </a>
                                                      @endif
                                                      @if ($commande->statut=="en_attente")
                                                          <a href="{{ route('servir.edit',$commande->id) }}" title="Modifier commande" type="button" class="btn btn-info btn-xs me-1">
